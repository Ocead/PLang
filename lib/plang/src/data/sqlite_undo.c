...
#define SQLITE_UNDO_ERRMSG_TABLE_MUST_BE_TEXT "Table name must be a text string"
...
static char *sqlite_undo_append_update_column_trigger(char *triggers, char *table, char *column) {
    char *r;
...
static char *sqlite_undo_prepend_update_table_trigger(char *triggers, char *table, char *columns) {
    char *r;
...
static char *sqlite_undo_prepend_delete_trigger(char *triggers, char *table, char *columns, char *values) {
    char *r;
    ...
    r = sqlite3_mprintf("CREATE TEMP TRIGGER _u_%s_d BEFORE DELETE ON %s "
                        "WHEN (SELECT undoable_active())=1 "
                        "BEGIN "
                        "UPDATE " SQLITE_UNDO_TABLE " "
                        "SET sql=sql ||"
...
static char *sqlite_undo_prepend_insert_trigger(char *triggers, char *table) {
    ...
    r = sqlite3_mprintf("CREATE TEMP TRIGGER _u_%s_i AFTER INSERT ON %s "
                        "WHEN (SELECT undoable_active())=1 "
                        "BEGIN "
                        "UPDATE " SQLITE_UNDO_TABLE " "
                        "SET sql=sql || "
...
static char *sqlite_undo_get_table_undo_triggers(sqlite3 *db, char *table, SqliteUndoUpdateType update_type) {
    int rc, pk;
    char *sql, *name, *delete_names = NULL, *delete_values = NULL, *update_columns = NULL, *triggers = NULL;
    sqlite3_stmt *stmt;
...
*/
    ...
    while ((rc = sqlite3_step(stmt)) != SQLITE_DONE) {
        ...
        delete_names  = sqlite_undo_add_delete_column(delete_names, name);
        delete_values = sqlite_undo_add_delete_value(delete_values, name);

        if (pk) { continue; }

        switch (update_type) {
            case SqliteUndoUpdateTypeColumn:
                triggers = sqlite_undo_append_update_column_trigger(triggers, table, name);
                break;
            case SqliteUndoUpdateTypeTable:
                update_columns = sqlite_undo_add_update_column(update_columns, name);
                break;
    ...
    if (rc == SQLITE_DONE) {
        if (update_type == SqliteUndoUpdateTypeTable) {
            triggers = sqlite_undo_prepend_update_table_trigger(triggers, table, update_columns);
        }

        triggers = sqlite_undo_prepend_delete_trigger(triggers, table, delete_names, delete_values);
...
static void sqlite_undo_undoable_table(sqlite3_context *context, int argc, sqlite3_value **argv) {
    int rc;
    ...
    if ((sqlite3_value_type(argv[0]) != SQLITE_TEXT)) {
        sqlite3_result_error(context, SQLITE_UNDO_ERRMSG_TABLE_MUST_BE_TEXT, -1);
        return;
    }

    update_type = (SqliteUndoUpdateType) sqlite3_value_int(argv[1]);

    if ((update_type != SqliteUndoUpdateTypeNone) && (update_type != SqliteUndoUpdateTypeTable) &&
        (update_type != SqliteUndoUpdateTypeColumn)) {

        sqlite3_result_error(context, SQLITE_UNDO_ERRMSG_INVALID_UPDATE_TYPE, -1);
        return;
    }
    ...
    if (!triggers) {
        sqlite3_result_error(context, SQLITE_UNDO_ERRMSG_CREATE_TRIGGER_FAILED, -1);
        return;
    }
...
static sqlite_int64 sqlite_undo_get_buffer_status(sqlite3 *db, char U_R) {
    ...
    if (rc == SQLITE_ROW) { c = sqlite3_column_int64(stmt, 0); }
...
static void sqlite_undo_undoable_begin(sqlite3_context *context, int argc, sqlite3_value **argv) {
    int rc;
    sqlite3 *db;

    if (sqlite_undo_undoable_active_flag != 0) {
        sqlite3_result_error(context, SQLITE_UNDO_ERRMSG_UNDOABLE_ACTIVE, -1);
        return;
    }
    ...
    if (rc != SQLITE_OK) {
        sqlite3_exec(db, "ROLLBACK TO SAVEPOINT " SQLITE_UNDO_SAVEPOINT_UNDOABLE, NULL, NULL, NULL);
        sqlite3_result_error_code(context, rc);
    } else {
        sqlite_undo_undoable_active_flag = 1;
    }

    sqlite3_exec(db, "RELEASE SAVEPOINT " SQLITE_UNDO_SAVEPOINT_UNDOABLE, NULL, NULL, NULL);
}

static int sqlite_undo_undoable_active(sqlite3_context *context, int argc, sqlite3_value **argv) {
    sqlite3_result_int(context, sqlite_undo_undoable_active_flag);
}

static void sqlite_undo_undoable_end(sqlite3_context *context, int argc, sqlite3_value **argv) {
    char *result;
    sqlite3 *db;
    ...
    if (sqlite_undo_undoable_active_flag) {
        ...
        sqlite3_result_error(context, SQLITE_UNDO_ERRMSG_UNDOABLE_NOT_ACTIVE, -1);
        return;
    }
}

static int sqlite_undo_step_get_transaction_rowid(sqlite3 *db, char U_R, sqlite_int64 *rowid) {
    int rc;
    sqlite3_stmt *stmt;
    ...
    if (rc == SQLITE_ROW) { *rowid = sqlite3_column_int64(stmt, 0); }
...
static int sqlite_undo_step_get_transaction_sql(sqlite3_context *context, sqlite_int64 rowid, char **sql) {
    int rc;
    sqlite3 *db;
    sqlite3_stmt *stmt;
    ...
    rc = sqlite3_prepare_v2(db, "SELECT sql FROM " SQLITE_UNDO_TABLE " WHERE rowid=?", -1, &stmt, NULL);
    ...
    if (rc == SQLITE_OK) {
        ...
        if (rc == SQLITE_ROW) {
            *sql = sqlite3_mprintf("%s", sqlite3_column_text(stmt, 0));
            rc   = SQLITE_OK;
        } else {
            *sql = NULL;
        }
...
static int sqlite_undo_step_delete_transaction(sqlite3_context *context, sqlite_int64 rowid) {
    int rc;
    sqlite3 *db;
    sqlite3_stmt *stmt;
    ...
    rc = sqlite3_prepare_v2(db, "DELETE FROM " SQLITE_UNDO_TABLE " WHERE rowid=?", -1, &stmt, NULL);
...
static void sqlite_undo_do(sqlite3_context *context, SqliteUndoOrRedo undo_or_redo) {
    int rc;
    ...
    assert(context);
    assert(undo_or_redo == SqliteUndoUndo || undo_or_redo == SqliteUndoRedo);

    db = sqlite3_context_db_handle(context);

    rc = sqlite_undo_step_get_transaction_rowid(db, undo_or_redo == SqliteUndoUndo ? 'U' : 'R', &rowid);

    if (rc != SQLITE_ROW) { goto result_error; }
    ...
    if (rc != SQLITE_OK) { goto result_error; }
    ...
    rc = sqlite3_exec(db, "SAVEPOINT " SQLITE_UNDO_SAVEPOINT_UNDO, NULL, NULL, NULL);
    if (rc != SQLITE_OK) { goto result_error; }

    rc = sqlite_undo_step_delete_transaction(context, rowid);
    if (rc != SQLITE_DONE) { goto rollback; }

    rc = sqlite_undo_step_prep_log(context, undo_or_redo == SqliteUndoUndo ? 'R' : 'U');
    if (rc != SQLITE_DONE) { goto rollback; }
    ...
    sqlite3_exec(db, "RELEASE SAVEPOINT " SQLITE_UNDO_SAVEPOINT_UNDO, NULL, NULL, NULL);
...
static void sqlite_undo_undo(sqlite3_context *context, int argc, sqlite3_value **argv) {
    sqlite_undo_do(context, SqliteUndoUndo);
}

static void sqlite_undo_redo(sqlite3_context *context, int argc, sqlite3_value **argv) {
...
static
    ...
    rc = sqlite3_exec(db, "CREATE TEMP TABLE " SQLITE_UNDO_TABLE "(sql TEXT, status TEXT)", NULL, NULL, NULL);
